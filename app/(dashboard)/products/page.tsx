...
export default function ProductsPage() {
  const [items, setItems]         = useState<BladeProduct[]>([]);
  ...
  useEffect(() => {
    Promise.all([getBladeProducts(), getCrateTypes()])
      .then(([p, c]) => { setItems(p); setCrateTypes(c); })
      .catch((err) => { setError(err.message); showToast("Error loading products", "error"); })
  ...
  async function confirmDelete() {
    if (!deleteId) return;
    try {
      await deleteBladeProduct(deleteId);
  ...
  return (
    <div className="px-6 py-5 flex gap-5">
      ...
      <div className="flex-1 min-w-0 space-y-4">
        {loading && <div className="text-xs text-zinc-500 bg-zinc-900 border border-zinc-800 px-4 py-2.5 rounded-lg">Loading products…</div>}
