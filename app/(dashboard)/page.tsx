...
export default async function OverviewPage() {
  ...
  const kpiCards = [
    { title: "Total Materials", value: data.totalMaterials.toString(), change: "in inventory", up: true, accent: "text-blue-400" },
    { title: "Open Orders", value: data.openOrders.toString(), change: "pending fulfillment", up: true, accent: "text-amber-400" },
    { title: "Low Stock Items", value: data.lowStockItems.toString(), change: "needs attention", up: data.lowStockItems === 0, accent: data.lowStockItems === 0 ? "text-emerald-400" : "text-red-400" },
    { title: "Production Capacity", value: `${data.productionCapacity}%`, change: "material availability", up: true, accent: "text-emerald-400" },
  ];
    ...
    <div className="px-8 py-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <section className="bg-zinc-900 rounded-xl border border-zinc-800">
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium border ${data.lowStockItems > 0 ? "bg-red-900 text-red-300 border-red-800" : "bg-emerald-900 text-emerald-300 border-emerald-800"}`}>
              ...
              data.lowStockAlerts.map((item) => (
                <li key={item.name} className="px-6 py-4">
                  ...
                  <div className="w-full bg-zinc-800 rounded-full h-1.5">
                    <div className="bg-red-500 h-1.5 rounded-full" style={{ width: `${Math.min(item.pct, 100)}%` }} />
                  </div>
                  <p className="text-xs text-zinc-600 mt-1">{Math.min(item.pct, 100)}% of threshold</p>
                </li>
              ))
            ) : (
              <li className="px-6 py-8 text-center text-xs text-zinc-500">All materials at or above minimum threshold</li>
            )}
          </ul>
        </section>
      </div>
      ...
      <section className="bg-zinc-900 rounded-xl border border-zinc-800">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-zinc-800">
          ...
          <h2 className="text-sm font-semibold text-zinc-100">System Insights</h2>
        ...
        <ul className="divide-y divide-zinc-800">
          {data.systemAlerts.map((alert, i) => (
            <li key={i} className="flex items-start gap-4 px-6 py-4">
              <span className={`mt-0.5 text-sm shrink-0 ${alert.type === "warning" ? "text-amber-400" : "text-zinc-400"}`}>
                {alert.type === "warning" ? "⚠" : "•"}
              </span>
