...
import {
  getOrders, addOrder, updateOrder, deleteOrder, changeOrderStatus,
  calculateOrderLines, getCustomerOptions, getBladeProductOptions,
...
export default function OrdersPage() {
  ...
  const [bladeProducts, setBP]      = useState<BPOption[]>([]);
  ...
  useEffect(() => {
    Promise.all([getOrders(), getBladeProductOptions(), getCustomerOptions()])
      .then(([o, bp, c]) => { setItems(o); setBP(bp); setCustomers(c); })
      .catch((e) => { setError(e.message); showToast("Error loading orders", "error"); })
  ...
  return (
    <div className="px-6 py-5 flex gap-5">
      <div className="flex-1 min-w-0 space-y-4">
        {loading && <div className="text-xs text-zinc-500 bg-zinc-900 border border-zinc-800 px-4 py-2.5 rounded-lg">Loading orders…</div>}
        {error && <div className="text-xs text-red-400 bg-zinc-900 border border-red-900 px-4 py-2.5 rounded-lg">{error}</div>}
